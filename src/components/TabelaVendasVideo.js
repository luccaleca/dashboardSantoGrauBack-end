...
export default function TabelaVendasVideo() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:8080/lucca/dashboard_vendas/vendas/venda_video_chamada.php?intervalo=semana');
  ...
  return (
    <TableContainer component={Paper}>
      <Table className={classes.table} size="small" aria-label="a dense table">
        <TableHead>
          <TableRow>
            <TableCell>Video-chamada</TableCell>
